...
"""
    ...
    panel = Panel(
        details,
        title=f"Prompt Details: {prompt.agent_name}",
        border_style="cyan"
    )
    console.print(panel)
...
def cmd_export(args):
    ...
    try:
        ...
        console.print("The system will automatically load your customized prompts on next run.")
...
def cmd_compare(args):
    ...
    if msg1 != msg2:
        differences.append(("system_message", f"{len(msg1)} chars", f"{len(msg2)} chars"))
...
def cmd_init(args):
    ...
    if output_dir.exists() and any(output_dir.iterdir()):
        if not args.force:
            console.print(f"[yellow]Warning:[/yellow] Directory '{output_dir}' already exists and is not empty.")
    ...
    try:
        ...
        console.print("  4. Run your analysis - custom prompts will be loaded automatically")
    except Exception as e:
