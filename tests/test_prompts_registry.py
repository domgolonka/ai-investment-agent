...
import pytest
from src.prompts import AgentPrompt, PromptRegistry, get_registry, get_prompt, export_prompts

def test_agentprompt_defaults_and_metadata():
    p = AgentPrompt(agent_key="t1", agent_name="T1", version="1.0", system_message="hello", metadata=None)
...
def test_export_and_roundtrip(tmp_path):
    ...
    reg.prompts["custom_agent"] = AgentPrompt(agent_key="custom_agent", agent_name="Custom", version="0.1", system_message="hi")
    export_dir = tmp_path / "exported"
    export_prompts(str(export_dir))
    # Expect files in export_dir
    assert (export_dir / "market_analyst.json").exists() or (export_dir / "fundamentals_analyst.json").exists()
