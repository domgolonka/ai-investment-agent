...
class TestParallelExecution:
    ...
    async def test_all_sources_called(self, fetcher):
        ...
        fetcher._fetch_yfinance_enhanced = AsyncMock(return_value={'symbol': 'TEST', 'currentPrice': 100})
        fetcher._fetch_yahooquery_fallback = MagicMock(return_value={'pe': 15}) # Not async
...
class TestSmartMerge:
    ...
    def test_quality_priority(self, fetcher):
        ...
        source_results = {
            'yfinance': {'trailingPE': 10.0, '_trailingPE_source': 'yfinance_info'},  # Quality 9
...
class TestTavilyGapFilling:
    ...
    async def test_tavily_triggered_low_coverage(self, fetcher):
        ...
        fetcher._fetch_all_sources_parallel = AsyncMock(return_value={
    ...
    async def test_tavily_skipped_high_coverage(self, fetcher):
        ...
        full_data = {
            ...
            'trailingPE': 15, 'marketCap': 100, 'priceToBook': 2,
            'returnOnEquity': 0.15, 'revenueGrowth': 0.1,
            'profitMargins': 0.2, 'debtToEquity': 0.5,
            'currentRatio': 1.5, 'freeCashflow': 1000,
            'operatingCashflow': 1000, 'numberOfAnalystOpinions': 5
        }
...
class TestPanicMode:
    ...
    async def test_dangerous_fields_filtering(self, fetcher):
        ...
        fetcher.tavily_client.search = MagicMock(return_value={
            'results': [{'content': 'ROE is 15%'}]
        })
        ...
        if fetcher.tavily_client.search.called:
            ...
            for call in fetcher.tavily_client.search.call_args_list:
                ...
                assert not all(term in query.lower() for term in ['p/e', 'price', 'market cap'])
